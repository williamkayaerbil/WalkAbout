  ...
  Walkabout.displayCategories = function(categories) {
    ...
    if (categories) {
      $.each(categories, function(catId, category) {
        ...
        if (category.type == "1") {
          catIcon = (CategoryImages) ? '<img src="' + wroot + "api/get-icon?r=0&g=0&b=0&icon=" + CategoryImages + '" alt="Events: ' + category.title + '">' : '';
          ...
          catIcon = (CategoryImages) ? '<img src="' + wroot + "api/get-icon?r=0&g=0&b=0&icon=" + CategoryImages + '" alt="Places: ' + category.title + '">' : '';
        ...
        if(place.events && place.events.length){
          $.each(place.events,function(id,ev){
              ...
              cat = self.categories["c_"+catId];
                if(cat){
                  urlMarkers[cat.image]=wroot + 'api/get-icon?r=255&g=255&b=255&icon='+cat.image;    
                }
          });
        }
